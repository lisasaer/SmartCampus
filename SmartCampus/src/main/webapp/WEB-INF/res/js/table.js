...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util'], function(exports){
  ...
  ,thisTable = function(){
    ...
    return {
      ...
      ,refresh: function() { //束建云添加用于只刷新当前分页数据，不重新渲染整个表格
      	that.refresh.call(that);
      }
      ,reloadExt: function(options) { //束建云添加区别于原reload，不重新渲染整个表格
  ...
  Class.prototype.render = function(){
    ...
    //束建云添加
    if(options.maxHeight && /^full-\d+$/.test(options.maxHeight)){
      that.fullHeightGap = options.maxHeight.split('-')[1];
      options.maxHeight = _WIN.height() - that.fullHeightGap;
    }
  ...
  Class.prototype.renderData = function(res, curr, count, sort){
    ...
    if(data.length === 0){
      ...
      that.layMain.find('tbody').html('');
      that.layHeader.css('overflow', 'auto');//束建云添加用于修复无数据时表头没有滚动条导致显示不全
      that.layMain.find('.'+ NONE).remove();
      return that.layMain.append('<div class="'+ NONE +'">'+ options.text.none +'</div>');
    }
    else { //束建云添加else
    	that.layHeader.css('overflow', 'hidden');
    	that.layMain.find('.'+ NONE).remove();
    }
      ...
      options.page = $.extend({
        ...
        ,jump: function(obj, first){
          if(!first){
            ...
            that.config.limit = obj.limit; //更新每页条数
            
            that.pullData(obj.curr);
          }
        }
      }, options.page);
      options.page.curr = curr; //束建云添加
  ...
  Class.prototype.loading = function(hide){
    ...
    if(options.loading){
      if(hide){
        that.loadindex && layer.close(that.loadindex);
        delete that.loadindex;
  ...
  Class.prototype.fullSize = function(){
    var that = this
    ,options = that.config
    ,height = options.height||options.maxHeight, bodyHeight;
    ...
    if(that.fullHeightGap){
      ...
      if(options.height){
      	...
      	that.elem.css('max-height', height);
      }
      //束建云修改end
    }
    ...
    if(options.height){
	  ...
	  that.layMain.css('max-height', bodyHeight);
	}
  };
    ...
    that.layTool.on('click', '*[lay-event]', function(e){
      ...
      ,openPanel = function(sets){
        ...
        //束建云添加
        	...
        	panel.css('max-height', options.maxHeight - (that.layTool.outerHeight() || 50));
        }
