...
$(function () {
    ...
    if (-1 == iRet) {
        ...
        if (window.confirm('您还未安装过插件，双击开发包目录里的WebComponentsKit.exe安装！')) {
            // window.open('WebComponentsKit.exe');
            window.location.href('../../res/hkwebsdk/websdk/codebase/WebComponentsKit.exe');
            return true;
        } else {
            return false;
        }
    ...
    WebVideoCtrl.I_InitPlugin("100%", "100%", {
        ...
        cbSelWnd: function (xmlDoc) {
            ...
            console.log("当前窗口索引=====" + g_iWndIndex);
        ...
        cbInitPluginComplete: function () {
            ...
            WebVideoCtrl.I_InsertOBJECTPlugin("divPlugin");   //写入插件
            // 检查插件是否最新
...
function getWindowSize() {
    ...
    console.log("nWidth==" + nWidth + "  nHeight==" + nHeight)
...
// 设置本地参数-wpp-3-12重写此方法
function ZyclickSetLocalCfg() {
    var arrXml = [],
        szInfo = "";
...
// 获取本地参数-wpp-3-12重写此方法-预览图片录像保存地址
function ZyclickGetLocalCfg() {
    var xmlDoc = WebVideoCtrl.I_GetLocalCfg();
...
// 获取本地参数-wpp-3-12重写此方法-回放图片录像保存地址
function ZyclickGetLocalCfgReplay() {
    var xmlDoc = WebVideoCtrl.I_GetLocalCfg();
    if (xmlDoc != null) {
        $("#playbackPicPath").val($(xmlDoc).find("PlaybackPicPath").eq(0).text());
        $("#downloadPath").val($(xmlDoc).find("DownloadPath").eq(0).text());
...
// 设置本地参数-wpp-3-12重写此方法-回放保存地址设置
function ZyclickSetLocalCfgReplay() {
    var arrXml = [],
        szInfo = "";
...
function changeWndNum(iType) {
    ...
    console.log("aaaaaaaaaa==" + WebVideoCtrl.I_ChangeWndNum(iType))
}
...
//wpp 2020-3-10重写此方法
// 登录
function ZyclickLogin(szIP, szPort, szUsername, szPassword) {
    var bTure = false;
    if ("" == szIP || "" == szPort) {
        return false;
    }
    ...
    var iRet = WebVideoCtrl.I_Login(szIP, 1, szPort, szUsername, szPassword, {
        success: function (xmlDoc) {
            ...
            setTimeout(function () {
                //$("#ip").val(szDeviceIdentify);
                getZyChannelInfo(szIP);
                getZyDevicePort(szIP);
            }, 10);
        },
        error: function (status, xmlDoc) {
            bTure = false;
...
//wpp-2020-3-13重写此方法
// 获取通道
function getZyChannelInfo(szIP) {
...
// 获取端口
function getZyDevicePort(zip) {
    var szDeviceIdentify = zip;
...
function clickStartRealPlay(iStreamType) {
    ...
    var startRealPlay = function () {
        WebVideoCtrl.I_StartRealPlay(szDeviceIdentify, {  //开始预览
            iRtspPort: iRtspPort,   //RTSP端口
...
//wpp 20202-3-10重写此方法
// 开始预览
function ZyclickStartRealPlay(ip, port, iChannelIDNo, bZeroChannel, iStreamType) {
    var oWndInfo = WebVideoCtrl.I_GetWindowStatus(g_iWndIndex),  //获取播放窗口信息
        szDeviceIdentify = ip,
        iRtspPort = port,
        szInfo = "";
    ...
    if ("undefined" === typeof iStreamType) {
        //iStreamType = parseInt($("#streamtype").val(), 10);
        iStreamType = iStreamType; //wpp默认主码流
    }
...
//wpp 20202-3-10重写此方法
// 开始预览
function ZyAllclickStartRealPlay(ip, port, iChannelIDNo, bZeroChannel, iStreamType, index) {
    //alert(index);
    var oWndInfo = WebVideoCtrl.I_GetWindowStatus(index),  //获取播放窗口信息
        szDeviceIdentify = ip,
        iRtspPort = port,
        szInfo = "";
...
//wpp 2020-3-17全部停止
function ZyclickAllStopRealPlay() {
    for (var a = 0; a < 16; a++) {
        var szInfo = "";
        var oWndInfo = WebVideoCtrl.I_GetWindowStatus(a);
...
function ZymouseDownPTZControl(iPTZIndex, ptz) {
    var oWndInfo = WebVideoCtrl.I_GetWindowStatus(g_iWndIndex),
        //bZeroChannel = $("#channels option").eq($("#channels").get(0).selectedIndex).attr("bZero") == "true" ? true : false,
        bZeroChannel = false;
    //iPTZSpeed = $("#ptzspeed").val();
...
function clickRecordSearch(iType) {
    var szDeviceIdentify = $("#ip").val(),
        iChannelID = parseInt($("#channels").val(), 10),
        bZeroChannel = $("#channels option").eq($("#channels").get(0).selectedIndex).attr("bZero") == "true" ? true : false,  //是否为零通道
        ...
        success: function (xmlDoc) {
            ...
            if ("MORE" === $(xmlDoc).find("responseStatusStrg").eq(0).text()) {
                ...
                for (var i = 0, nLen = $(xmlDoc).find("searchMatchItem").length; i < nLen; i++) {
                    ...
                    // console.log("more_szStartTime=" + szStartTime);
                    // console.log("more_szEndTime=" + szEndTime);
                    // console.log("more_szFileName=" + szFileName);
                ...
                for (var i = 0; i < iLength; i++) {
                    ...
                    //将这些视频文件一条一条的插入到数据库中
                    console.log("ok_szStartTime=" + szStartTime);
                    console.log("ok_szEndTime=" + szEndTime);
                    console.log("ok_szFileName=" + szFileName);
                    var Size = xmlDoc.valueOf();
                    console.log("xmlDoc=" + Size);
