    ...
    if (playParams && playParams.hasOwnProperty('decoderPath')) {
      ...
      if (!/^ezopen:\/\//.test(this.opt.currentSource)) {
        ...
        if (!/^ezopen:\/\//.test(this.opt.currentSource)) {
          ...
          return;
        } else if (com.zy.SmartCampus.lorasearch(/(.hls|.m3u8|.wss|.flv|.rtmp)/) !== -1 && !/.live(.hls|.m3u8|.wss|.flv|.rtmp)/.test(this.opt.currentSource)) {
          throw new Error('请指定正确的播放协议');
          return;
        } else if (com.zy.SmartCampus.lorasearch(/(.hls|.m3u8|.wss|.flv|.rtmp)/) === -1 && !/[a-z\d]{32}(\.hd)?\.live$/.test(this.opt.currentSource)) {
