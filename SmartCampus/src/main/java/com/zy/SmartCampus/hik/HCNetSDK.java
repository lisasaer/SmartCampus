...
public interface HCNetSDK extends StdCallLibrary {
    ...
    String classPath = HCNetSDK.class.getResource("/").getPath();
    String dllPath = classPath.substring(1,classPath.indexOf("classes"))+"hklib/HCNetSDK.dll";

    HCNetSDK INSTANCE = (HCNetSDK) Native.loadLibrary(dllPath.replace("/","\\"), HCNetSDK.class);

    //HCNetSDK INSTANCE = (HCNetSDK) Native.loadLibrary("E:\\javaweb\\project\\TestDemo\\SmartCampus\\src\\main\\webapp\\WEB-INF\\hklib\\HCNetSDK.dll", HCNetSDK.class);

    //HCNetSDK INSTANCE = (HCNetSDK) Native.loadLibrary(ClientDemo.DLL_PATH+"hklib\\HCNetSDK.dll", HCNetSDK.class);

    /***宏定义***/
    //常量
    public static final int MAX_NAMELEN = 16;    //DVR本地登陆名
    public static final int COMM_PASSNUM_INFO_ALARM = 0x5201;    //DVR本地登陆名
...
//车牌识别（NET_VCA_PLATE_CFG）;
    ...
    public static final int NET_DVR_JSON_CONFIG = 2550;   //获取设备人员
    ...
    public static final int NET_DVR_SET_FINGERPRINT_CFG = 2151;    //设置人脸参数
    public static final int NET_DVR_GET_FINGERPRINT_CFG = 2150;    //删除人脸参数
    ...
    public static final int MAX_ALARMHOST_ALARMOUT_NUM = 512;    //采集指纹信息
    /**********************设备类型 end***********************/
...
//zy
    ...
    public static  class  NET_DVR_EVENT_CARD_LINKAGE_CFG_V51 extends  Structure{
        ...
        public byte[]       byAlarmInSetup= new  byte[MAX_ALARMHOST_ALARMOUT_NUM];
        public byte[]       byAlarmInClose= new  byte[MAX_ALARMHOST_ALARMOUT_NUM];
        public byte[]       byReaderStopBuzzer= new  byte[MAX_ALARMHOST_ALARMOUT_NUM];
    ...
    public static class NET_DVR_PASSNUM_INFO_ALARM extends Structure {
        ...
        public byte[] byNetUser = new byte[MAX_NAMELEN];
...
//NET_DVR_Login()参数结构
    ...
    public static class NET_DVR_FINGER_PRINT_INFO_CTRL extends Structure {
        ...
        public NET_DVR_DEL_FINGER_PRINT_MODE struProcessMode;  //处理方式
    ...
    public static class NET_DVR_FINGER_PRINT_BYCARD extends Structure {
        ...
        public byte[] byFingerPrintID = new byte[MAX_FINGER_PRINT_NUM];        //需要删除的人脸编号，按数组下标，值表示0-不删除，1-删除该人脸
    ...
    public static class NET_DVR_FINGER_PRINT_BYREADER extends Structure {
        ...
        public byte[] byRes1 = new byte[3];       //保留
        public byte[] byCardNo = new byte[ACS_CARD_NO_LEN]; //人脸关联的卡号
        public byte[] byRes = new byte[100];          //保留
    }
    ...
    public static class getPerson extends Structure{
    ...
    public static final int MAX_FINGER_PRINT_NUM = 10;
    ...
    //门禁人员查询条件
    public static class UserInfoSearchCode extends Structure{
    ...
    public static class NET_DVR_FACE_PARAM_STATUS extends Structure {
        public int dwSize;
        public byte[] byCardNo = new byte[ACS_CARD_NO_LEN]; //人脸关联的卡号
        public byte[] byCardReaderRecvStatus = new byte[MAX_CARD_READER_NUM_512];  //人脸读卡器状态，按字节表示，0-失败，1-成功，2-重试或人脸质量差，3-内存已满，4-已存在该人脸，5-非法人脸ID，6-算法建模失败，7-未下发卡权限，8-未定义（保留），9-人眼间距小，10-图片数据长度小于1KB，11-图片格式不符（png/jpg/bmp）,12-图片像素数量超过上限，13-图片像素数量低于下限，14-图片信息校验失败，15-图片解码失败，16-人脸检测失败，17-人脸评分失败
...
//2007-11-30 V211支持以下接口 //11
    ...
    public static class NET_DVR_XML_CONFIG_INPUT extends Structure {
        public byte[] byRes1 = new byte[31];            //保留
        public int dwSize ;                //结构体大小
        public byte lpRequestUrl ;                //请求信令，字符串格式
        public int dwRequestUrlLen ;                        //请求信令长度，字符串长度
        public byte lpInBuffer ;                        //输入参数缓冲区，XML格
        public byte dwInBufferSize ;                        //输入参数缓冲区大小
        public short dwRecvTimeOut  ;                    //接收超时时间，单位：ms，填0则使用默认超时5s
        public byte byForceEncrpt;                    //是否强制加密（启用之后透传的XML报文将加密传输，AES128加密算法）：0- 否，1- 是
    }
    NativeLong NET_DVR_STDXMLConfig(NativeLong lUserID, Pointer lpOutBuff, Pointer dwOutBuff);
...
interface PlayCtrl extends StdCallLibrary {
    PlayCtrl INSTANCE = (PlayCtrl) Native.loadLibrary("E:\\lib\\PlayCtrl",
            PlayCtrl.class);
